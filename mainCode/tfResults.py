...
ax.set_xlabel('Days Since Jan 1st {0}'.format(testingYear))
...
for w in windows:
	...
	rw = moving_average(r, w)
	tw = moving_average(t, w)/(365./12)
	f = plt.figure(figsize=(12, 8))
	ax = plt.subplot(111)
	ylabel = 'CO Field (Moving Average {0} Days, ppbv)'.format(w/24)
	ax.set_xlabel('Months Since Jan 1st {0}'.format(testingYear))
	ax.set_ylabel(ylabel)
	ax.set_title('Fit')
